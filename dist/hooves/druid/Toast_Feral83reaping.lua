...
__exports.registerDruidFeralToast83reaping = function(OvaleScripts)
do
	local name = "toast_feral83"
	local desc = "[Toast][8.3] Druid: Feralreaping"
	local code = [[
...
AddFunction feralcooldownsmainactions
{
 ...
 if buffpresent(tigers_fury_buff) spell(concentrated_flame_essence)
}
...
AddFunction feralcooldownsshortcdactions
{
 ...
 unless { enemies(tagged=1) > 5 or 600 > 45 } and spell(thorns)
 {
  ...
  unless buffpresent(tigers_fury_buff) and spell(concentrated_flame_essence)
  {
   #ripple_in_space,if=buff.tigers_fury.up
   #if buffpresent(tigers_fury_buff) spell(ripple_in_space_essence)
   #worldvein_resonance,if=buff.tigers_fury.up
   if buffpresent(tigers_fury_buff) spell(worldvein_resonance_essence)
   ...
   if target.timetodie() < 1.5 or { target.healthpercent() > 80 or target.healthpercent() <= 20 } and reaping_delay() > 29 or target.timetohealthpercent(20) > 30 and reaping_delay() > 44 spell(reaping_flames_essence)
  }
 }
}
...
{
 { enemies(tagged=1) > 5 or 600 > 45 } and spell(thorns) or buffpresent(tigers_fury_buff) and spell(concentrated_flame_essence)
}
 ...
 unless { enemies(tagged=1) > 5 or 600 > 45 } and spell(thorns) or { buffpresent(reckless_force_buff) or buffpresent(tigers_fury_buff) } and spell(the_unbound_force)
 {
  ...
  unless combopoints() == 0 and spell(feral_frenzy)
  {
   ...
   unless { enemies(tagged=1) > 5 or 600 > 60 } and spell(purifying_blast)
   {
    ...
    if buffpresent(tigers_fury_buff) spell(guardian_of_azeroth)

    unless buffpresent(tigers_fury_buff) and spell(concentrated_flame_essence) or buffpresent(tigers_fury_buff) and spell(ripple_in_space_essence) or buffpresent(tigers_fury_buff) and spell(worldvein_resonance_essence) or { target.timetodie() < 1.5 or { target.healthpercent() > 80 or target.healthpercent() <= 20 } and reaping_delay() > 29 or target.timetohealthpercent(20) > 30 and reaping_delay() > 44 } and spell(reaping_flames_essence)
    {
...
{
 { enemies(tagged=1) > 5 or 600 > 45 } and spell(thorns) or { buffpresent(reckless_force_buff) or buffpresent(tigers_fury_buff) } and spell(the_unbound_force) or combopoints() == 0 and spell(feral_frenzy) or { enemies(tagged=1) > 5 or 600 > 60 } and spell(purifying_blast) or buffpresent(tigers_fury_buff) and spell(concentrated_flame_essence) or buffpresent(tigers_fury_buff) and spell(ripple_in_space_essence) or buffpresent(tigers_fury_buff) and spell(worldvein_resonance_essence) or { target.timetodie() < 1.5 or { target.healthpercent() > 80 or target.healthpercent() <= 20 } and reaping_delay() > 29 or target.timetohealthpercent(20) > 30 and reaping_delay() > 44 } and spell(reaping_flames_essence)
}
   ...
   unless { buffpresent(scent_of_blood_feral) or damage(swipe_cat) * enemies(tagged=1) > damage(rake) + target.lastdamage(rake_debuff) * 5 } and spellusable(swipe_cat) and spellcooldown(swipe_cat) < timetoenergyfor(swipe_cat)
   {
    ...
    unless { not target.debuffpresent(rake_debuff) or not hastalent(bloodtalons_talent) and target.debuffremaining(rake_debuff) < baseduration(rake_debuff) * 0.3 and target.timetodie() > 4 } and spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake)
    {
     ...
     unless hastalent(bloodtalons_talent) and buffpresent(bloodtalons_buff) and target.debuffremaining(rake_debuff) <= 7 and persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) * 0.85 and target.timetodie() > 4 and spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake)
     {
      ...
      if buffpresent(tigers_fury_buff) and 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies(tagged=1) * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) spell(brutal_slash)
